...
async function main() {
  ...
  await checkAsync('Clean v0 migration reaches schema v9 and preserves compatibility columns', async () => {
    const adapter = new ExpoSQLiteAdapter();
    try {
      await migrationModule(adapter, calendarDate).runMigrations();
      assert.equal(adapter.getFirstSync('SELECT version FROM _schema_version').version, 9);
