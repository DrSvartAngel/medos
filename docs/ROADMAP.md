...
- **Core Architecture Invariants:**
  ...
  - **Academic Spine:** `Committee` → `Subject` → `Topic` → `Material`.
  - **Shared Context Contract:** Decouples academic identity, browsing context, activity scope, evidence attribution, entry point, and return destination.
  - **AI Integration:** Contextual and global `Ask MedOS` assistant capability (`Inform` / `Generate` / `Propose`), not a primary navigation product.
...
- **Phase 14 — Design System & Application Shell Rebuild:** 🟡 READY TO START
  - *Phase 14.1:* Visual foundations and semantic tokens (Neutral Zen palette, Manrope typography, semantic tokens, theme store)
...
- Full Regression Suite:
  ...
  - Total: 309 automated checks passing.(9 checks)
