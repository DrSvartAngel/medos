...
- **Phase 14 — Design System & Application Shell Rebuild:** 🟡 IN PROGRESS
  ...
  - *Phase 14.6:* Primary Navigation Shell (Today / Study / Review / Plan rebuild) (NEXT UP)
...
- **Checklist Summary:**
  ...
  - *Phase 14.5 (26 checks):* Phone safe areas, tablet safe areas, phone page gutter, tablet page gutter, large-tablet gutter, tablet content width, max-width behavior, scroll integrity, no nested-scroll regression, no double top safe area, no double bottom safe area, phone bottom clearance, tablet rail geometry, tablet main workspace geometry, optional inspector slot, inspector hidden on phone, modal phone presentation, modal tablet presentation, sheet safe area, orientation resize stability, light theme shell surfaces, dark theme shell surfaces, system theme shell surfaces, shared primitives inside shell, user flows reachable, route/nav integrity.
...
- **RagAnswerCard Component** (`components/study-ai/RagAnswerCard.tsx`):
  - Presentation component for source-grounded answers generated by the RAG pipeline.
  - Interactive citation badges mapped directly to retrieved chunks (`RagCitation`), resolving `[SRC-N]` markers.
  - Evidence state banners for `supported`, `partial`, and `insufficient` states with distinct color coding and guidance.
  - Medical safety disclaimer, copy-to-clipboard action, generated timestamp, and token/source count indicators.
- **Topic Assistant UI Integration** (`app/topics/[id]/assistant.tsx`):
  - Added dedicated `rag` tab ("Ask Sources" / "Kaynaklara Sor" or Ask MedOS mode) in the study assistant.
  - Topic-wide source scope: automatically gathers and retrieves relevant chunks across all study sources belonging to the topic.
  - Invokes `ragAnswerService.generateAnswer` with robust progress/loading state, user question input, and clear error presentation.
- **Localization** (`i18n/en.ts`, `i18n/tr.ts`):
  - Full 1:1 bilingual parity across all new RAG UI strings (tab label, question placeholder, ask action, disclaimer, evidence badges, empty and insufficient evidence notices).
- **Validation** (`scripts/validate-phase12-step8.cjs`):
  - Static structural validation verifying component exports, service wiring in assistant, evidence handling, and localization tokens: **PASS** (`Phase 12.8 validation passed.`).
  - TypeScript compilation (`tsc --noEmit`): **PASS (0 errors)**.
- **Physical QA Status**:
